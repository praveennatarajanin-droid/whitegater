...
export default function OrganizationsPage() {
  ...
  const fetchTenancyData = async () => {
    ...
    try {
      ...
      if (orgsData.length > 0) {
        ...
        // 2. Fetch Members
        const memRes = await fetch(`http://localhost:8000/api/v1/organizations/${targetOrg.id}/members`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (memRes.ok) setMembers(await memRes.json());

        // 3. Fetch Projects
        const projRes = await fetch(`http://localhost:8000/api/v1/organizations/${targetOrg.id}/projects`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleInviteMember = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/v1/organizations/${activeOrg.id}/members`, {
        method: 'POST',
        headers: {
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/v1/organizations/${activeOrg.id}/projects`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleRemoveMember = async (userId: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/v1/organizations/${activeOrg.id}/members/${userId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
